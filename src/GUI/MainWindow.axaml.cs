...
using OperaSuprema.Core.Infrastructure;
...
namespace OperaSuprema.GUI
{
	...
	private readonly AutonomousCrawler _crawler;
        ...
        public MainWindow()
        {
            ...
            // --- INIZIALIZZAZIONE CATENA DELLA MEMORIA NVME E RELAZIONALE ---
            ...
            var btnExportDossier = this.FindControl<Button>("BtnExportDossier");
            if (btnExportDossier != null) btnExportDossier.Click += OnExportDossierClicked;
	...
	// --- MOTORE MENU SUPERIORE ---
        ...
        private async void OnExportDossierClicked(object? sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Title = "Esporta Dossier di Sessione",
                    SuggestedFileName = $"Dossier_{_currentSession.Title.Replace(" ", "_")}_{DateTime.Now:yyyyMMdd}.md",
                    DefaultExtension = "md",
                    FileTypeChoices = new[]
                    {
            ...
            {
                AppendToChat($"[ERRORE EXPORT]: {ex.Message}", Brushes.Red);
            }
            finally
            {
        ...
        private async void OnSendButtonClicked(object? sender, RoutedEventArgs? e = null)
        {
            ...
            try
            {
                ...
                // --- MEMBRANA 3: SALVATAGGIO UTENTE E ESTRAZIONE DECISIONI ---
                string userMsgId = Guid.NewGuid().ToString();
                int userTokens = userText.Length / 4;
                await _ledgerService.InsertChatMessageAsync(userMsgId, _currentSession.Id, "user", userText, userTokens);

                var userDecisionMatch = System.Text.RegularExpressions.Regex.Match(userText, @"(?i)(?:decisione:?|impostiamo|impostat[oa])\s+(?:l[aeoi]\s+|il\s+|lo\s+)?(.+?)\s+(?:impostat[oa]\s+)?(?:su[llaeoi\']*|in|come|è|deve essere)\s+([^,\.\n]+)");
                if (userDecisionMatch.Success)
                {
                    string subject = userDecisionMatch.Groups[1].Value.Trim();
                    string objectValue = userDecisionMatch.Groups[2].Value.Trim();
                    // Rimuove eventuali articoli determinativi residui all'inizio del soggetto
                    subject = System.Text.RegularExpressions.Regex.Replace(subject, @"^(il|lo|la|i|gli|le)\s+", "", System.Text.RegularExpressions.RegexOptions.IgnoreCase).Trim();
                    await _ledgerService.RegisterOrUpdateDecisionAsync(new ExtractedTriple(subject, "impostato_su", objectValue, "Scelta imposta dall'utente"), _currentSession.Id, userMsgId);
                }
                // -------------------------------------------------------------
        ...
        private async Task InvokeArchitectAsync(string userText, long currentTelegramChatId, bool useVoiceForThisChain)
        {
            ...
            // --- MEMBRANA 3: INIEZIONE CONTESTO DETERMINISTICO (Prefix Caching) ---
            var query = new RetrievalQuery(userText, _memoryRouter.FastExtractKeywords(userText), _currentSession.Id);
            var contextPayload = await _memoryRouter.RouteAndAssembleAsync(query);
            
            if (!string.IsNullOrEmpty(contextPayload.DeterministicPrefixBlock) && contextPayload.DeterministicPrefixBlock.Contains("- "))
            {
                contextData = contextPayload.DeterministicPrefixBlock + "\n\n" + contextData;
            }

            if (contextPayload.RetrievedSnippets != null && contextPayload.RetrievedSnippets.Count > 0)
            {
            ...
            // --- MEMBRANA 3: SALVATAGGIO ASSISTANT E ESTRAZIONE DECISIONI ---
            string botMsgId = Guid.NewGuid().ToString();
            int botTokens = masterAnalysis.Length / 4;
            await _ledgerService.InsertChatMessageAsync(botMsgId, _currentSession.Id, "assistant", masterAnalysis, botTokens);

            var botDecisionMatch = System.Text.RegularExpressions.Regex.Match(masterAnalysis, @"(?i)(?:decisione:?|impostiamo|impostat[oa])\s+(?:l[aeoi]\s+|il\s+|lo\s+)?(.+?)\s+(?:impostat[oa]\s+)?(?:su[llaeoi\']*|in|come|è|deve essere)\s+([^,\.\n]+)");
            if (botDecisionMatch.Success)
            {
                string subject = botDecisionMatch.Groups[1].Value.Trim();
                string objectValue = botDecisionMatch.Groups[2].Value.Trim();
                // Rimuove eventuali articoli determinativi residui all'inizio del soggetto
                subject = System.Text.RegularExpressions.Regex.Replace(subject, @"^(il|lo|la|i|gli|le)\s+", "", System.Text.RegularExpressions.RegexOptions.IgnoreCase).Trim();
                await _ledgerService.RegisterOrUpdateDecisionAsync(new ExtractedTriple(subject, "impostato_su", objectValue, "Decisione architetturale da Master Mentor"), _currentSession.Id, botMsgId);
            }
            // ----------------------------------------------------------------
                ...
                while ((line = await reader.ReadLineAsync()) != null)
                {
                    if (line.StartsWith("data: ") && line.Substring(6) != "[DONE]")
                    {
                        ...
                        if (delta.TryGetProperty("content", out var contentElement))
                        {
                            ...
                            try 
                            {
                                string safeText = System.Text.RegularExpressions.Regex.Unescape(fullResponse.ToString());
                                Dispatcher.UIThread.Post(() => { aiMessageBlock.Text = safeText; scrollViewer?.ScrollToEnd(); });
                            }
                            catch { }
                        }
                    }
...
Metti i comandi in un blocco codice ```bash. Non aggiungere altre spiegazioni.";
	...
	// =========================================================================
        ...
        {
            try { fullText = System.Text.RegularExpressions.Regex.Unescape(fullText); } catch { }

            if (!Avalonia.Threading.Dispatcher.UIThread.CheckAccess())
            {
                Avalonia.Threading.Dispatcher.UIThread.Post(() => AppendToChat(fullText, color, showCopy, copyPrefixToRemove));
                return (new SelectableTextBlock(), new StackPanel());
            }
	...
	// =========================================================================
        ...
        private void OnLensToggled(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
        {
            var activeToggle = sender as Avalonia.Controls.Primitives.ToggleButton;
            ...
            if (activeToggle != lensSintesi && lensSintesi != null) lensSintesi.IsChecked = false;
            if (activeToggle != lensAnalisi && lensAnalisi != null) lensAnalisi.IsChecked = false;
            if (activeToggle != lensEstrai && lensEstrai != null) lensEstrai.IsChecked = false;

            if (activeToggle.IsChecked == true)
            {
                var txtInput = this.FindControl<Avalonia.Controls.TextBox>("UserInputTextBox");
                if (txtInput != null && string.IsNullOrWhiteSpace(txtInput.Text))
                {
                    txtInput.Text = $"Esegui: {activeToggle.Content}";
...
5. VINCOLI ARCHITETTURALI E TECNOLOGICI (LEDGER ENFORCED):
   - Conformati TASSATIVAMENTE alle decisioni elencate nel blocco '### ARCHITECTURE ACTIVE DECISIONS'. Quel blocco ha priorità assoluta su qualsiasi impostazione predefinita.
   - Se nel blocco non è specificato diversamente per il framework UI, il target predefinito è <TargetFramework>net10.0</TargetFramework>.
   - Rispetta rigorosamente i pattern (MVC/MVP/MVVM) e le librerie grafiche stabilite nelle decisioni attive.
   - REGOLA ANTI-PIGRIZIA: Imponi SEMPRE al Coder di restituire i file sorgente PER INTERO, dalla prima all'ultima riga, senza mai troncarli.
...
{coderLTM}";
	...
	// ==========================================================
        ...
        {
            var decisions = await _ledgerService.GetAllDecisionsAsync();
            if (decisions.Count == 0)
            {
                AppendToChat("[DECISION LEDGER]: Nessuna decisione registrata nel database.", Avalonia.Media.Brushes.Orange);
                return;
            }

            var active = System.Linq.Enumerable.ToList(System.Linq.Enumerable.Where(decisions, d => d.Status == "Active"));
            var superseded = System.Linq.Enumerable.ToList(System.Linq.Enumerable.Where(decisions, d => d.Status == "Superseded"));

            AppendToChat($"=== 🏛️ REGISTRO DECISIONI ARCHITETTURALI ({decisions.Count} TOTALI) ===", Avalonia.Media.Brushes.Gold);

            if (active.Count > 0)
            {
                AppendToChat($"🟢 DECISIONI ATTIVE (SYSTEM ENFORCED - {active.Count}):", Avalonia.Media.Brushes.SpringGreen);
                foreach (var d in active)
                {
                    AppendToChat($"  • {d.Subject} [{d.Predicate}] {d.ObjectValue}" + (!string.IsNullOrEmpty(d.Rationale) ? $" (Motivo: {d.Rationale})" : ""), Avalonia.Media.Brushes.LightGreen);
                }
            }

            if (superseded.Count > 0)
            {
                AppendToChat($"\n🔴 DECISIONI REVOCATE / SUPERATE ({superseded.Count}):", Avalonia.Media.Brushes.OrangeRed);
                foreach (var d in superseded)
                {
                    AppendToChat($"  <s>• {d.Subject} [{d.Predicate}] {d.ObjectValue}</s>", Avalonia.Media.Brushes.Gray);
                }
            }
